   ...
   $.fn.monthpicker = function(options){
      ...
      validElems.each(function(){
         ...
         $(this).focus(function(){
            ...
            $(document).bind('click.hideMonthpicker',function(e,exception){
               if(e.button == 2) return true;
               if(exception === undefined)
                  $(document).unbind("click.hideMonthpicker");
               $('body>div[class*="monthpicker-"]').not(exception).trigger("close");
               //field.blur();
            }).trigger('click.hideMonthpicker',p[0]);
         })
         .click(function(){
            if(!$(this).data("monthpicker-widget").is(':animated') && !$(this).data("monthpicker-widget").is(':visible'))
               $(this).focus();
         })
         .keydown(function(e){
