//Monthpicker 1.0 - Production Version (by Jon Torodash). See https://github.com/jon-torodash/jqmonthpicker for more information.;(function($){    
;(function($) {
  $.fn.monthpicker = function(options) {
    ...
    if(options.destroy == "destroy") {
      validElems.each(function() {
        $(this).data("monthpicker-widget", null)
      });
      return this
    }
    var dummy = $("<div></div>");
    dummy.css("color", "#5A5A5A");
    dummy.css("color", options.colorDefault);
    options.colorDefault = dummy.css("color");
    dummy.css("color", "#bebebe");
    dummy.css("color", options.colorHover);
    options.colorHover = dummy.css("color");
    dummy.css("color", "#0099FF");
    dummy.css("color", options.colorSelected);
    options.colorSelected = dummy.css("color");
    dummy.css("color", "#DDDDDD");
    dummy.css("color", options.colorBknd);
    options.colorBknd = dummy.css("color");
    dummy.css("color", "#FFFFFF");
    dummy.css("color", options.colorMonthTxt);
    options.colorMonthTxt = dummy.css("color");
    dummy.css("color", "#000000");
    dummy.css("color", options.colorYearTxt);
    options.colorYearTxt = dummy.css("color");
    dummy.css("font-size", "12px");
    dummy.css("font-size", options.fontSize);
    options.fontSize = dummy.css("font-size");
    dummy.css("font-family", "sans-serif");
    dummy.css("font-family", options.font);
    options.font = dummy.css("font-family");
    if(typeof options.submitText != "string") {
      options.submitText = "Enter"
    }
    ...
    if(options.minPoint != null && options.maxPoint != null && options.minPoint > options.maxPoint) {
      options.minPoint = null;
      options.maxPoint = null
    }
    validElems.each(function() {
      var picker = $("<div></div>");
      picker.data("input", $(this)).css({"display":"none", "position":"absolute", "border":"solid 1px #5A5A5A", "padding":"5px", "background-color":options.colorBknd, "font-family":options.font, "font-size":options.fontSize, "font-weight":"bold"});
      if(options.type == "double") {
        picker.append(createOne("from").css("margin-bottom", "10px")).append(createOne("to")).attr("class", "monthpicker-double")
      }else {
        picker.append(createOne("to")).attr("class", "monthpicker-single")
      }
      $(this).data("monthpicker-widget", picker).data("value", "");
      picker.bind("close", function() {
        var picker = $(this);
        ...
        if(picker.hasClass("monthpicker-single")) {
          field.val(todate)
        }else {
      ...
      $(this).focus(function() {
        ...
        p.appendTo("body").css({"left":$(this).offset().left + "px", "top":$(this).offset().top + $(this).height() + 6 + "px"}).click(function(e) {
          e.stopPropagation();
          return false
        }).fadeIn(200, function() {
        ...
        switch(e.which) {
          ...
          ;
          case 18:
            $(this).data("monthpicker-funckey", false);
            break
        }
        return true
      }).blur(function(e) {
        $("body").trigger("click.hideMonthpicker")
      }).click(function(e) {
        if($(this).data("monthpicker-widget").data("ready") != true) {
        }
        e.stopPropagation();
    ...
    function forbiddenChars(key, origInput) {
      if(origInput.data("monthpicker-funckey") == true) {
        return false
      }
      switch(key) {
        case 16:
        ...
        ;
        case 18:
          origInput.data("monthpicker-funckey", true);
          return false;
          break;
        case 13:
          $("body").trigger("click.hideMonthpicker");
    ...
    function createOne(id) {
      ...
      var div = $('<div class="monthpicker-' + id + '"></div').css({"text-align":"center"});
      div.append('<input class="monthpicker-scroll prev" type="button" style="float:left;width:25px;padding:0" value="<<"/>').append('<input class="monthpicker-scroll next" type="button" style="float:right;width:25px;padding:0" value=">>"/>').append('<div class="curr" style="display:inline-block;width:35px;padding-top:5px;margin-left:auto;margin-right: auto;text-align:center;color:' + options.colorYearTxt + '">' + maxYear + "</div>").append('<div class="months-' + id + '" style="clear:both"></div>').css({"width":"270px"}).data("date", 
      null);
      $.each(["Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec"], function(i, e) {
        if(i == 6) {
          div.find(".months-" + id).append("<br/>")
        }
        var month = $('<span class="month-' + id + '">' + e + "</span>").css({"display":"inline-block", "background-color":options.colorDefault, "width":"35px", "cursor":"pointer", "height":"16px", "margin":"5px", "color":options.colorMonthTxt, "text-align":"center"}).data({"value":i + 1, "field":id}).hover(function() {
          if(!$(this).hasClass("selected") && !$(this).hasClass("out_of_range")) {
            $(this).css("background-color", options.colorHover)
          }
        }, function() {
          if(!$(this).hasClass("selected")) {
            $(this).css("background-color", options.colorDefault)
          }
        });
        div.find(".months-" + id).append(month)
      });
      if(id == "to") {
        var set = $('<input type="button" value="' + options.submitText + '"/>').click(function() {
          $(this).closest(".monthpicker-single,.monthpicker-double").trigger("close")
        }).appendTo(div);
        var clr = $('<input type="button" value="Clear"/>').click(function() {
          var div = $(this).closest(".monthpicker-single,.monthpicker-double");
          div.find('[id^="months"]').data("value", null).find('[class^="month"]').removeClass("selected").css({"background-color":options.colorDefault});
      ...
      div.find(".monthpicker-scroll").click(function(e) {
        ...
        if($(this).hasClass("next") && (options.maxPoint == null || year.html() < options.maxPoint.getFullYear())) {
          ...
          if(picker.hasClass("monthpicker-double") && picker.find(".monthpicker-to .curr").html() < year.html()) {
          ...
          if($(this).hasClass("prev") && (options.minPoint == null || year.html() > options.minPoint.getFullYear())) {
            year.html(parseInt(year.html()) - 1);
            var picker = $(this).parent().parent();
            if(picker.hasClass("monthpicker-double") && picker.find(".monthpicker-from .curr").html() > year.html()) {
      ...
      div.find("*").andSelf().mousedown(function(e) {
        e.preventDefault()
      });
      div.find(".monthpicker-scroll").mousedown(function(e) {
        e.stopPropagation()
      });
      div.find("*").andSelf().mouseup(function(e) {
        e.stopPropagation()
      });
      return div
    }
    function monthClickHandler(e, params) {
      ...
      if(monthpicker.hasClass("monthpicker-single")) {
        if(!withinBounds($(this).data("value"), to.find(".curr").html())) {
          if(params != undefined && params.simulated) {
        ...
        if(!$(this).hasClass("selected") || params != undefined && params.simulated) {
          ...
          $(this).removeClass("selected").css({"background-color":options.colorDefault})
        }
        ...
        if($(this).data("field") == "from") {
          ...
          selectedTo = to.find(".month-to.selected");
          if(selectedTo.length > 0 && to.find(".curr").html() == from.find(".curr").html()) {
            if(selectedTo.data("value") < $(this).data("value")) {
          ...
          if(from.find(".selected").length > 0) {
            from.data("date", new Date($(this).data("value") + "/1" + "/" + from.find(".curr").html()))
          }else {
            from.data("date", null)
          }
        }else {
          ...
          if(selectedFrom.length > 0 && to.find(".curr").html() == from.find(".curr").html()) {
            if(selectedFrom.data("value") > $(this).data("value")) {
    ...
    function parseRange(val) {
      ...
      if(typeof val == "string" && val.split("/").length == 2) {
        ...
        if(isNaN(m) || isNaN(y) || Math.floor(m) > 12 || m < 1 || y < 0) {
          return null
        }else {
